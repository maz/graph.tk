...
(function(){
	...
	window.requestAnimationFrameOnce=function(func){
...
app.ui = (function () {
    ...
    */
	...
	function point_slope(ctx,m,px,py){
		ctx.beginPath();
		var x = boundleft;
		ctx.move(x, ((x + (-1*px)) * m) + py);
		for (var x = boundleft; x < boundright; x += (boundright - boundleft) / width) {
			ctx.line(x, ((x + (-1*px)) * m) + py);
		}
		ctx.stroke();
		ctx.stroke();
	}
	...
	function draw(){
		requestAnimationFrameOnce(actual_draw);
	}
    ...
    */
		...
		window.magical_hacked_point=[px,py];
		draw();
	}
